...
pub unsafe fn op_in<const OP: u8>(operand: u64) {
    asm!(
        ...
        // would have to move a value into that register, which is inefficient.
        //
        // The trick we use is to prepend '0' to the register's name to make it
...
unsafe impl super::super::AmxOps for AmxOps<'_> {
    #[inline(always)]
    unsafe fn ldx(&mut self, x: u64, ptr: *mut ()) {
        ldx(x | (ptr as u64 & const { 0x00FF_FFFF_FFFF_FFFF }));
    }
    #[inline(always)]
    unsafe fn ldy(&mut self, x: u64, ptr: *mut ()) {
        ldy(x | (ptr as u64 & const { 0x00FF_FFFF_FFFF_FFFF }));
    }
    #[inline(always)]
    unsafe fn stx(&mut self, x: u64, ptr: *mut ()) {
        stx(x | (ptr as u64 & const { 0x00FF_FFFF_FFFF_FFFF }));
    }
    #[inline(always)]
    unsafe fn sty(&mut self, x: u64, ptr: *mut ()) {
        sty(x | (ptr as u64 & const { 0x00FF_FFFF_FFFF_FFFF }));
    }
    #[inline(always)]
    unsafe fn ldz(&mut self, x: u64, ptr: *mut ()) {
        ldz(x | (ptr as u64 & const { 0x00FF_FFFF_FFFF_FFFF }));
    }
    #[inline(always)]
    unsafe fn stz(&mut self, x: u64, ptr: *mut ()) {
        stz(x | (ptr as u64 & const { 0x00FF_FFFF_FFFF_FFFF }));
    }
    #[inline(always)]
    unsafe fn ldzi(&mut self, x: u64, ptr: *mut ()) {
        ldzi(x | (ptr as u64 & const { 0x00FF_FFFF_FFFF_FFFF }));
    }
    #[inline(always)]
    unsafe fn stzi(&mut self, x: u64, ptr: *mut ()) {
        stzi(x | (ptr as u64 & const { 0x00FF_FFFF_FFFF_FFFF }));
    }
    #[inline(always)]
