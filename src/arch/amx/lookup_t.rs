...
/// A marker type for the `genlut` instruction's lookup table.
/// Implementers are in the form `(direction, index, value)`.
///
/// - `direction` is either [`Normal`] or [`Reverse`].
/// - `index` is the data type for indices.
/// - `value` is the data type for looked-up values.
pub trait LookupT {
    /// The raw lookuptable mode number for `genlut` instruction.
    fn genlut_mode(&self) -> u64;
}

/// Specifies the normal application of a lookup table.
pub struct Normal;

/// Specifies the reversed application of a lookup table.
pub struct Reverse;
...
#[cfg(feature = "either")]
impl<Left: LookupT, Right: LookupT> LookupT for either::Either<Left, Right> {
    #[inline]
    fn genlut_mode(&self) -> u64 {
        match self {
